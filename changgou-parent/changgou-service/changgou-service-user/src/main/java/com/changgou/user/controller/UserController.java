...
public class UserController {
    ...
    public Result login(String username, String password, HttpServletResponse response){
        ...
        if(password.equals(user.getPassword())){
            ...
            map.put("status", true);
            String jwt = JwtUtil.creatJwt(JSON.toJSONString(map), null);
            response.addCookie(new Cookie(JwtUtil.AUTHORIZE_TOKEN, jwt));
    ...
    public Result getUserInfo(HttpServletRequest request){
        String header = request.getHeader(JwtUtil.AUTHORIZE_TOKEN);
        System.out.println(header);
        if (StringUtils.isEmpty(header)){
            return new Result(true, StatusCode.OK, "failed", "not Auto");
        }
        JSONObject jsonObject = JSON.parseObject(header);
        Map<String, Object> map = new HashMap<>(jsonObject);
        User user = userService.findById((String) map.get("username"));
